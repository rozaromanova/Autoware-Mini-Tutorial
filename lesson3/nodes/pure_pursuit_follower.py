...
class PurePursuitFollower:
    ...
    def current_pose_callback(self, msg):
        if self.path_linestring is None or self.distance_to_velocity_interpolator is None:
            ...
            # Calculate the lookahead point and the required steering angle.
            ...
            steering_angle = np.arctan2(2.0 * self.wheel_base * np.sin(lookahead_heading - heading),ld)

            linear_velocity = float(self.distance_to_velocity_interpolator(d_ego_from_path_start))
            linear_acceleration = 0

        # Create and publish a VehicleCommand message.
